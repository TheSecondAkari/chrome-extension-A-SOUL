...
class DetailedFetchBlob {
  constructor(
    ...
    fetch = init.fetch || top.fetch
  ) {
    // Fire in the Fox fix
    if (this.firefoxConstructor(input, init, onprogress, onabort, onerror))
      return;
    ...
    this.blob = null;
    this.reader = null;
    this.blobPromise = fetch(input, init).then(
  ...
  firefoxConstructor(
    ...
    this.blobPromise = new Promise((resolve, reject) => {
      ...
      xhr.onload = () => {
        resolve((this.blob = new Blob(this.buffer)));
        this.buffer = null;
      };
      ...
      xhr.onerror = (e) => {
        this.onerror({ target: this, type: e.type });
        reject(e);
      };
